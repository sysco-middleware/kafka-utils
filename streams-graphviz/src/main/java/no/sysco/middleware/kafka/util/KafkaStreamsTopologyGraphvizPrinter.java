...
public class KafkaStreamsTopologyGraphvizPrinter {
  ...
  private static void printStores(TopologyDescription topologyDescription, StringWriter writer) {
    topologyDescription.subtopologies().stream()
        ...
        .forEach(topic ->
            writer.append("  \"store-")
                .append(topic)
                .append("\" [shape=box3d, label=\"Store ")
                .append(topic)
                .append("\"];\n"));
  }
  ...
  private static void printSubtopology(TopologyDescription.Subtopology subtopology,
    ...
    writer.append("    label = \"Sub-Topology ")
        .append(String.valueOf(subtopology.id()))
        .append("\";\n");
    ...
    for (TopologyDescription.Node node : subtopology.nodes()) {
      ...
      if (node instanceof TopologyDescription.Source) {
        ...
        Stream.of(topics.substring(1, topics.length() - 1).split(","))
            .map(String::trim)
            .forEach(topic -> writer.append("    \"topic-")
                .append(topic)
                .append("\" -> \"node-")
                .append(node.name())
                .append("\";\n"));
        ...
        writer.append("    \"node-")
            .append(node.name())
            .append("\" -> \"topic-")
            .append(((TopologyDescription.Sink) node).topic())
            .append("\";\n");
      }
    }
